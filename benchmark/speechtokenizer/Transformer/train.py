#!/usr/bin/env python3
"""
Training script for blendshape decoder with WavTokenizer
"""
import torch
...
def create_trainer(config):
    ...
    train_dataset = SpeechBlendshapeDataset(
        speech_tokens_path=config.get('train_tokens', 'speechtokenizer_tokens/train_utt2speech_token.pt'),
        beat2_dir=config['beat2_dir'],
        mask_categories=config.get('mask_categories', ['jaw', 'mouth'])
    )
    
    val_dataset = SpeechBlendshapeDataset(
        speech_tokens_path=config.get('val_tokens', 'speechtokenizer_tokens/val_utt2speech_token.pt'),
        beat2_dir=config['beat2_dir'],
        mask_categories=config.get('mask_categories', ['jaw', 'mouth'])
    )
    ...
    # Create model - WavTokenizer version
    model = BlendshapeDecoder(
        vocab_size=config.get('vocab_size', 1024),   # SpeechTokenizer vocab per RVQ level
        n_q=config.get('n_q', 8),                    # Number of RVQ levels
    ...
    criterion = BlendshapeLoss(
        ...
        smoothness_weight=config.get('smoothness_weight', 0.05)
    )
