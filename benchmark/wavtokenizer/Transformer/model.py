#!/usr/bin/env python3
"""
Transformer Decoder for WavTokenizer Speech Token to ARKit Blendshape prediction
"""
import torch
...
class BlendshapeDecoder(nn.Module):
    """
    Transformer decoder for WavTokenizer speech token to blendshape prediction
    ...
    def __init__(
        self,
        vocab_size=4096,  # WavTokenizer vocab size
        d_model=512,
        nhead=8,
        num_layers=6,
        ...
        """
        Args:
            vocab_size: Size of speech token vocabulary (4096 for WavTokenizer)
    ...
    def forward(self, speech_tokens, target_length=None, token_lengths=None):
        ...
        if target_length is None:
            # Inference: WavTokenizer is 75Hz, blendshapes are 30Hz
            # So target_length ≈ seq_len * (30/75) = seq_len * 0.4
            target_length = int(seq_len * 0.4)
