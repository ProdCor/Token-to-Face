#!/usr/bin/env python3
"""
Batch Inference script for the BlendshapeDecoder model with WavTokenizer tokens.

Generates ARKit blendshape sequences from ALL pre-extracted WavTokenizer speech tokens using a trained model.
"""
import argparse
import yaml
import torch
...
def calculate_target_length(num_tokens, target_fps, token_fps=75):
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Batch inference with WavTokenizer: Generate ARKit blendshapes for ALL utterances in a token file.",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the input WavTokenizer speech tokens file (.pt file containing all test utterances)."
    )
    parser.add_argument(
        '--output_dir',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=75,
        help="Token rate in Hz (default: 75 for WavTokenizer)."
    )
    parser.add_argument(
        '--device',
        type=str,
...
python inference_batch.py \
    --checkpoint checkpoints/cben_wavtokenizer_1000ep/eye_blink_eye_look_eye_shape_brow_cheek_nose_model_epoch_999.pth \
    --tokens ../data/wavtokenizer_tokens/test_utt2speech_token.pt \
    --output_dir results/test_set_results_cben
