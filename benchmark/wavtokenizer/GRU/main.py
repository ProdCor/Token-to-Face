...
from model_adapt import FaceDiffBeatWavTokenizer
...
def trainer_diff(args, train_loader, dev_loader, model, diffusion, optimizer, 
    ...
    for e in range(epoch + 1):
        ...
        for i, (audio, vertice, template, one_hot, file_name) in pbar:
            ...
            # Handle WavTokenizer tokens (same as CosyVoice2)
            if args.use_wavtokenizer:
...
def main():
    ...
    parser.add_argument("--model", type=str, default="face_diffuser_wavtokenizer", help='name of the trained model')
    parser.add_argument("--template_file", type=str, default="templates.pkl",
                        help='path of the train subject templates')
    parser.add_argument("--save_path", type=str, default="save/cben_100ep_GRU_WAV", help='path of the trained models')
    ...
    # WavTokenizer specific arguments
    parser.add_argument("--use_wavtokenizer", action='store_true',
                    help='Use pre-extracted WavTokenizer tokens instead of HuBERT')
    parser.add_argument("--token_path", type=str, default="wavtokenizer_tokens/",
                        help='Path to directory containing WavTokenizer token .pt files')
    parser.add_argument("--codebook_size", type=int, default=4096,
                        help='Size of WavTokenizer codebook (typically 4096)')
    parser.add_argument("--token_embedding_dim", type=int, default=512,
                        help='Embedding dimension for WavTokenizer tokens')
    ...
    # Print configuration
    print("\n" + "="*60)
    print("Training Configuration - WavTokenizer")
    print("="*60)
    ...
    if args.dataset == 'beat2':
        if args.use_wavtokenizer:
            model = FaceDiffBeatWavTokenizer(
                args,
            ...
            raise ValueError("This script requires --use_wavtokenizer flag for BEAT2")
    else:
