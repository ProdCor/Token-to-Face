...
def read_data_wavtokenizer(args):
    """
    Load BEAT2 data with pre-extracted WavTokenizer tokens
    Each chunk is treated as an independent sample
    """
    print("Loading BEAT2 data with WavTokenizer tokens...")
    ...
    for split in splits:
        # WavTokenizer token file naming: {split}_utt2speech_token.pt
        ...
        for key, tokens in tqdm(utt2tokens.items(), desc=f"Processing {split}"):
            # Token key format: "{utterance_id}_chunk_{chunk_idx}"
            # Vertices file format: "{split}_{utterance_id}_{chunk_idx_padded}.npy"
...
def read_data(args):
    ...
    # For BEAT2 with WavTokenizer tokens
    if args.dataset == 'beat2' and hasattr(args, 'use_wavtokenizer') and args.use_wavtokenizer:
        return read_data_wavtokenizer(args)
