...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio = self.data[index]["audio"]  # Now CosyVoice2 tokens (LongTensor)
        ...
        # No need for FloatTensor conversion like with HuBERT features
...
def read_data_cosyvoice2(args):
    """
    Load BEAT2 data with pre-extracted CosyVoice2 tokens
    Each chunk is treated as an independent sample
    """
    print("Loading BEAT2 data with CosyVoice2 tokens...")
    data = defaultdict(dict)
    train_data = []
    valid_data = []
    test_data = []
    
    # Load pre-extracted tokens
    ...
    # Load tokens for each split
    splits = ['train', 'val', 'test']
    split_data = {'train': train_data, 'val': valid_data, 'test': test_data}
    
    for split in splits:
        token_file = os.path.join(token_base_path, f'utt2speech_token_{split}.pt')
        ...
        # Process each utterance chunk
        for key, tokens in tqdm(utt2tokens.items(), desc=f"Processing {split}"):
            # Token key format: "train_1_wayne_0_111_111_chunk_0"
            # Vertices file format: "train_1_wayne_0_111_111_000.npy"
            ...
            # Remove "_chunk_{idx}" suffix and extract chunk index
            parts = key.rsplit('_chunk_', 1)
            if len(parts) != 2:
                print(f"Warning: Unexpected key format: {key}")
                continue
            
            base_name = parts[0]  # e.g., "train_1_wayne_0_111_111"
            chunk_idx = parts[1]  # e.g., "0"
            
            # Convert chunk index to zero-padded format (0 -> "000")
            chunk_idx_padded = str(chunk_idx).zfill(3)
            
            # Vertices file format: {base_name}_{chunk_idx_padded}.npy
            ...
            if not os.path.exists(vertices_full_path):
                ...
                print(f"  Token key was: {key}")
                print(f"  Base name: {base_name}")
                print(f"  Chunk idx: {chunk_idx} -> {chunk_idx_padded}")
                continue
            
            # Store data
            data[key]["audio"] = tokens_tensor  # Shape: (seq_len,)
            data[key]["vertice"] = vertices_full_path
            data[key]["name"] = key
            
            # Use neutral template for BEAT2
    ...
    # Create subjects dict (for BEAT2, we just use split names)
    subjects_dict = {
...
def read_data(args):
    ...
    # For BEAT2 with CosyVoice2 tokens
    if args.dataset == 'beat2' and hasattr(args, 'use_cosyvoice_tokens') and args.use_cosyvoice_tokens:
        return read_data_cosyvoice2(args)
