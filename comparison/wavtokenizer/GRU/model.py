...
class FaceDiffBeatWavTokenizer(nn.Module):
    """
    FaceDiffuser adapted for BEAT2 dataset with WavTokenizer tokens
    ...
    def __init__(
        ...
        self.i_fps = 75  # WavTokenizer token rate (75 Hz)
        self.o_fps = args.output_fps  # 30 fps for BEAT2
        self.one_hot_timesteps = np.eye(args.diff_steps)
        self.dropout_rate = dropout
        
        # WavTokenizer token embedding
        self.vocab_size = args.codebook_size    # 4096 from your config
        self.embedding_dim = args.token_embedding_dim  # 512 or custom
        ...
        print(f"FaceDiffBeatWavTokenizer Configuration:")
        print(f"  Vocab size (codebook): {self.vocab_size}")
    ...
    def forward(
        ...
        cond_embed: Tensor,  # WavTokenizer token indices (B, token_seq_len)
        ...
        # At 75 Hz tokens and 30 fps motion: we need 2.5x more tokens than motion frames
        ...
        # Interpolate from 75 Hz to 30 fps (downsample by 2.5x)
