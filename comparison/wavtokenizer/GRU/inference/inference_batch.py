#!/usr/bin/env python3
"""
Batch inference for trained WavTokenizer-based model using pre-extracted tokens
"""
import argparse
import os
import torch
...
def calculate_num_frames(num_tokens, output_fps, token_fps=75):
    """
    ...
    Args:
        ...
        token_fps: Token rate (default: 75 Hz for WavTokenizer)
...
def process_single_utterance(model, diffusion, tokens, args, device):
    ...
    max_valid_token = 4095
    if tokens.max() > max_valid_token:
        num_clipped = (tokens > max_valid_token).sum().item()
        logging.warning(f"Clipping {num_clipped} tokens from range [{tokens.min()}, {tokens.max()}] to [0, {max_valid_token}]")
        tokens = torch.clamp(tokens, 0, max_valid_token)
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch inference for GRU model using pre-extracted tokens',
    ...
    parser.add_argument("--token_fps", type=int, default=75,
                        help='Token rate (75 Hz for WavTokenizer)')
    parser.add_argument("--diff_steps", type=int, default=1000)
    parser.add_argument("--codebook_size", type=int, default=4096,
                        help='WavTokenizer codebook size')
    
    # Paths
    parser.add_argument("--save_path", type=str, default="save")
    parser.add_argument("--model", type=str, default="face_diffuser_wavtokenizer")
...
python inference_batch.py \
    ...
    --output_dir results/test_set_results_100ep_GRU_CV2 \
    --epoch 100

python inference_batch.py \
    --tokens wavtokenizer_tokens/test_utt2speech_token.pt \
    --output_dir results/test_set_results_100ep_GRU_WAV \
    --epoch 100
