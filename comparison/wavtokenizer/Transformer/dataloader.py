#!/usr/bin/env python3
"""
DataLoader for WavTokenizer Speech Tokens + ARKit Blendshapes
"""
import torch
import numpy as np
...
class SpeechBlendshapeDataset(Dataset):
    """
    Dataset pairing WavTokenizer speech tokens with ARKit blendshapes
    
    Pipeline:
    1. Load pre-extracted WavTokenizer tokens from .pt file
    2. Load FLAME params from BEAT2 (100 expressions + 3 jaw)
    3. Transform to ARKit blendshapes (51)
    4. Apply mask to keep only desired blendshapes (e.g., mouth+jaw)
    """
    ...
    def __init__(
        ...
        """
        Args:
            speech_tokens_path: Path to {split}_utt2speech_token.pt
    ...
    def _build_index(self):
        ...
        for chunk_key in self.speech_tokens.keys():
            ...
            # Use rsplit to handle cases like "1_wayne_0_1_1_chunk_0"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--speech_tokens_path', default='../data/wavtokenizer_tokens/train_utt2speech_token.pt',
                        help='Path to train/val/test_utt2speech_token.pt')
