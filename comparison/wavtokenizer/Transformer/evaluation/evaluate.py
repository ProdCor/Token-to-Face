    ...
    for pred_f in pred_files:
        try:
            ...
            if args.gt_format == "flame":
                ...
                print(f"GT raw shape: {gt_facial.shape}")
                print(f"GT raw range: [{gt_facial.min():.4f}, {gt_facial.max():.4f}]")
                print(f"GT after transform shape: {gt_seq_full.shape}")
                print(f"GT after transform range: [{gt_seq_full.min():.4f}, {gt_seq_full.max():.4f}]")
            else:
...
'''
python evaluate4.py --pred_path ../modular/results/test_set_results
'''