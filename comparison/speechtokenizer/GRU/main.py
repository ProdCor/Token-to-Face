...
from utils import plot_losses, create_gaussian_diffusion, get_arkit_mask
...
def trainer_diff(args, train_loader, dev_loader, model, diffusion, optimizer, 
    ...
    for e in range(epoch + 1):
        ...
        for i, (audio, vertice, template, one_hot, file_name) in pbar:
            ...
            # Handle Speechtokenizer tokens
            if args.use_speechtokenizer:
                # audio is (B, n_q, T) LongTensor
...
def main():
    ...
    parser.add_argument("--vertice_dim", type=int, default=51, help='number of vertices - 23370*3 for BIWI, 51 for ARKit blendshapes')
    ...
    parser.add_argument("--save_path", type=str, default="save/cben_100ep_GRU_SPT")
    ...
    # Speechtokenizer specific arguments
    parser.add_argument("--use_speechtokenizer", action='store_true',
                        help='Use pre-extracted SpeechTokenizer tokens instead of HuBERT')
    parser.add_argument("--token_path", type=str, default="speechtokenizer_tokens/",
                        help='Path to directory containing SpeechTokenizer token .pt files')
    parser.add_argument("--codebook_size", type=int, default=1024,
                        help='Size of SpeechTokenizer codebook per RVQ level')
    parser.add_argument("--n_q", type=int, default=8,
                        help='Number of RVQ quantizer levels')
    parser.add_argument("--token_embedding_dim", type=int, default=512,
                        help='Embedding dimension for speech tokens')
    ...
    # Print configuration
    print("\n" + "="*60)
    print("Training Configuration - SpeechTokenizer")
    print(f"Codebook Size (per RVQ level): {args.codebook_size}")
    print(f"RVQ Levels (n_q): {args.n_q}")
    print("="*60)
    ...
    print(f"Token Embedding Dim: {args.token_embedding_dim}")
    ...
    if args.dataset == 'beat2':
        if args.use_speechtokenizer:
            ...
            raise ValueError("This script requires --use_speechtokenizer flag for BEAT2")
    else:
        raise ValueError("This script is specifically for BEAT2 dataset")
